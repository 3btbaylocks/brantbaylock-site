...
loadContent().then(items=>{
 ...
 const r=document.querySelector('#resources-list');if(r){const a=items.filter(x=>x.contentType==='Resource');r.innerHTML=a.map(x=>{const href=x.internalUrl||x.downloadUrl||'#';const ext=/^https?:/.test(href);return `<div class="resource-card"><div><div class="eyebrow">${esc(x.eyebrow||x.category)}</div><h3>${esc(x.title)}</h3><p>${esc(x.summary)}</p></div><a class="btn ghost" href="${esc(href)}" ${ext?'target="_blank" rel="noopener"':''}>${esc(x.ctaLabel||'Open')}</a></div>`}).join('')}
