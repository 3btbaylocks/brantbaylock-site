...
function resourceHref(x){if(x.internalUrl)return x.internalUrl;if(x.slug==='owners-and-trusted-advisors')return '/resources/owners-and-trusted-advisors.html';return x.downloadUrl||x.externalUrl||'#'}
function card(item){const href=item.internalUrl||item.downloadUrl||item.externalUrl||('#');const ext=/^https?:/.test(href);return `<a class="card" href="${esc(href)}" ${ext?'target="_blank" rel="noopener"':''}><span class="tag">${esc(item.eyebrow||item.category||item.contentType)}</span><h3>${esc(item.title)}</h3><p>${esc(item.summary)}</p><span class="more">${esc(item.ctaLabel||'Read more')}</span></a>`}
loadContent().then(items=>{
 ...
 const r=document.querySelector('#resources-list');if(r){const a=items.filter(x=>x.contentType==='Resource');r.innerHTML=a.map(x=>{const href=resourceHref(x);const ext=/^https?:/.test(href);return `<div class="resource-card"><div><div class="eyebrow">${esc(x.eyebrow||x.category)}</div><h3>${esc(x.title)}</h3><p>${esc(x.summary)}</p></div><a class="btn ghost" href="${esc(href)}" ${ext?'target="_blank" rel="noopener"':''}>${esc(x.ctaLabel||'Open')}</a></div>`}).join('')}
 const c=document.querySelector('#corridor-current');if(c){const x=items.find(x=>x.contentType==='Corridor Edition'&&x.currentEdition==='Yes')||items.find(x=>x.contentType==='Corridor Edition');c.innerHTML=x?`<div class="eyebrow">Current edition · ${esc(fmtDate(x.publishDate))}</div><h3>${esc(x.title)}</h3><p>${esc(x.summary)}</p><div class="btn-row"><a class="btn gold" href="${esc(x.downloadUrl)}" target="_blank" rel="noopener">${esc(x.ctaLabel||'Read current edition')}</a></div>`:'<strong>Current edition unavailable.</strong>'}
 const archive=document.querySelector('#corridor-archive');if(archive){const a=items.filter(x=>x.contentType==='Corridor Edition'&&x.currentEdition!=='Yes');archive.innerHTML=a.map(card).join('')}
}).catch(console.error);
